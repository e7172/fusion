...
def merge_video(target_path : str, output_video_resolution : str, output_video_fps : Fps, output_video_watermark : bool) -> bool:
	...
	if output_video_watermark:
		commands = [ '-r', str(temp_video_fps), '-i', temp_frames_pattern, '-i', watermark, '-s', str(output_video_resolution), '-c:v', state_manager.get_item('output_video_encoder') ]
		if not os.path.isfile(watermark): logger.error(f"Font file not found: {watermark}", __name__)
	else:
